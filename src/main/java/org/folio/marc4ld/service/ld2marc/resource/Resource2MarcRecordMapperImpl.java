...
public class Resource2MarcRecordMapperImpl implements Resource2MarcRecordMapper {
  ...
  private List<DataField> getFields(Resource resource, PredicateDictionary predicate,
    ...
    // In the future, we may need the parent  of this resource also to determine if mapping can be done.
    // If needed, we can enhance this method to receive the parent resource also in the future.
    var mapperOptional = marc4ldMappers.stream()
            .filter(mapper -> mapper.canMap(predicate, resource))
            .findFirst();
    if (mapperOptional.isPresent()) {
      // do mapping using the mapper bean
      return mapperOptional.get().map2marc(resource);
    } else {
      // do mapping using the YAML configuration
