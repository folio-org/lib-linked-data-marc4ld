...
public class Ld2MarcExtentMapper implements Ld2MarcMapper {
  ...
  private boolean hasExtentRelation(ResourceEdge edge) {
    return EXTENT.equals(edge.getPredicate())
      && Set.of(ResourceTypeDictionary.EXTENT).containsAll(edge.getTarget().getTypes());
  }
  ...
  private void addLabels(Resource extent, DataField dataField) {
    addRepeatableSubfield(extent, dataField, LABEL.getValue(), A);
  }

  private void addPhysicalDescription(Resource instance, DataField dataField) {
    addNonRepeatableSubfield(instance, dataField, PHYSICAL_DESCRIPTION.getValue(), B);
  }

  private void addDimensions(Resource instance, DataField dataField) {
    addRepeatableSubfield(instance, dataField, DIMENSIONS.getValue(), C);
  }

  private void addAccompanyingMaterial(Resource instance, DataField dataField) {
    addNonRepeatableSubfield(instance, dataField, ACCOMPANYING_MATERIAL.getValue(), E);
  }

  private void addMaterialSpecified(Resource extent, DataField dataField) {
    addNonRepeatableSubfield(extent, dataField, MATERIALS_SPECIFIED.getValue(), THREE);
  }

  private void addNonRepeatableSubfield(Resource resource, DataField dataField, String property, char subfield) {
    MarcUtil.addNonRepeatableSubfield(resource, dataField, property, subfield, marcFactory);
  }

  private void addRepeatableSubfield(Resource resource, DataField dataField, String property, char subfield) {
    MarcUtil.addRepeatableSubfield(resource, dataField, property, subfield, objectMapper, marcFactory);
  }
}
