...
import org.folio.marc4ld.configuration.property.Marc4LdNormalizationRules;
...
public class MarcRecordNormalizerImpl implements MarcRecordNormalizer {

  private final Marc4LdNormalizationRules marc4LdNormalizationRules;
  ...
  private void normalizeSubfields(List<Subfield> subfields, String tag) {
    ...
    for (int i = 1; i < subfields.size(); i++) {
      ...
      generateLookups(tag, subfields.get(i).getCode())
        .stream()
        .map(lookup -> marc4LdNormalizationRules.getSubfieldRules().get(lookup))
  ...
  private void normalizeLastSubfield(List<Subfield> subfields, String tag) {
    ...
    ofNullable(marc4LdNormalizationRules.getLastSubfieldRules().get(tag))
