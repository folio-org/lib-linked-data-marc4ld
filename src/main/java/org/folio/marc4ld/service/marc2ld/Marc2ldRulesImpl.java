...
public class Marc2ldRulesImpl implements Marc2ldRules {
  ...
  private final Map<String, Collection<Marc2ldFieldRule>> rules;
  ...
  @Override
  public Collection<Marc2ldFieldRule> findFiledRules(String tag) {
    return rules.getOrDefault(tag, Collections.emptyList());
  }

  private Map<String, Collection<Marc2ldFieldRule>> initRules(Marc4BibframeRules marc4BibframeRules) {
    return marc4BibframeRules.getFieldRules()
      .entrySet()
      .stream()
      .collect(Collectors.toMap(Map.Entry::getKey, entry -> createRules(entry.getValue())));
  }

  private Collection<Marc2ldFieldRule> createRules(List<Marc4BibframeRules.FieldRule> fieldRules) {
    return fieldRules.stream()
      .map(this::createRule)
      .toList();
  }

  private Marc2ldFieldRule createRule(Marc4BibframeRules.FieldRule rule) {
    var builder = Marc2ldFieldRuleImpl.builder()
      .fieldRule(rule)
  ...
  private Optional<Marc2ldFieldRule.Relation> getRelation(Marc4BibframeRules.FieldRule rule) {
    return Optional.of(rule)
  ...
  private Collection<Marc2ldFieldRule> getEdges(Marc4BibframeRules.FieldRule rule) {
    return Optional.of(rule)
  ...
  public static class RelationImpl implements Marc2ldFieldRule.Relation {
