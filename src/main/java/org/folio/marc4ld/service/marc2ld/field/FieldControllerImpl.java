...
public class FieldControllerImpl implements FieldController {
  ...
  public void handleField(
    ...
    var fieldMapper = getMapper(dataField, controlFields, fieldRule);
    var mappedResources = fieldMapper.createResources(parent);

    mappedResources
      .forEach(resource -> additionalMapping(resource, fieldRule, dataField, controlFields));
  }

  private void additionalMapping(
    Resource resource,
    Marc2ldFieldRule fieldRule,
    DataField dataField,
    List<ControlField> controlFields
  ...
  private static boolean isMapping(Marc2ldFieldRule fieldRule, Marc2ldMapper mapper) {
    return Optional.of(fieldRule)
      .map(Marc2ldFieldRule::getPredicate)
      .map(mapper::canMap)
      .orElse(false);
  }

  private FieldMapper getMapper(DataField dataField, Collection<ControlField> controlFields, Marc2ldFieldRule rule) {
